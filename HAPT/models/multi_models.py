...
from models.trans_s2s import Encoder_s2s
...
class model_HAPT(nn.Module):
    def __init__(self,batchsize,device,hidden_size,num_layers,bidirectional,window_size,how='s2l'):
        ...
        if how == 's2l':
            ...
            # self.linears.append(nn.ReLU())
            # self.linears.append(nn.Flatten())
            # self.linears.append(nn.Dropout(0.4))
        ...
        ##output [B,250,12]
            ...
            self.linears.append(nn.Tanh())
            self.linears.append(nn.Dropout(0.2))
            self.linears.append(nn.Conv1d(in_channels=self.coeffienct*hidden_size,out_channels=self.coeffienct*hidden_size,kernel_size=30,padding='same'))
            self.linears.append(nn.BatchNorm1d(num_features=self.coeffienct*hidden_size)),
            self.linears.append(nn.ReLU())
            self.linears.append(nn.Dropout(0.2))
            self.linears.append(nn.Conv1d(in_channels=self.coeffienct*hidden_size,out_channels=12,kernel_size=30,padding='same'))
            self.linears.append(nn.BatchNorm1d(num_features=12)),
            self.linears.append(Rearrange('b c l -> b l c'))
            self.linears.append(nn.Softmax(dim=-1))
...
class model_gru_HAPT(nn.Module):
    def __init__(self,batchsize,device,hidden_size,num_layers,bidirectional,window_size,how):
        ...
        ##output [B,12]
            ...
            self.linears.append(nn.Dropout(0.2))
            self.linears.append(
                nn.Conv1d(in_channels=self.coeffienct * hidden_size, out_channels=self.coeffienct * hidden_size,
                          kernel_size=30, padding='same'))
            self.linears.append(nn.BatchNorm1d(num_features=self.coeffienct * hidden_size)),
            self.linears.append(nn.ReLU())
            self.linears.append(nn.Dropout(0.2))
            self.linears.append(
                nn.Conv1d(in_channels=self.coeffienct * hidden_size, out_channels=12, kernel_size=30, padding='same'))
            self.linears.append(nn.BatchNorm1d(num_features=12)),
            self.linears.append(Rearrange('b c l -> b l c'))
            self.linears.append(nn.Softmax(dim=-1))
...
class model_transformer_HAPT(nn.Module):
    def __init__(self,batchsize,
        ...
        if how == 's2l':
            ...
            self.trans_classifier = Encoder_s2s(
                sequence_length=250,
                patch_length=10,
                num_classes=12,
                token_dim=600,
