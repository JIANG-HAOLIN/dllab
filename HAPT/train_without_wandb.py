...
if dataset == 'HAPT':
    ...
    elif structure == 'cnn_lstm_cnn':
        mdl = cnn_lstm_cnn_HAPT(batchsize=batch_size, device=device, hidden_size=hidden_size,
                         num_layers=num_layers, bidirectional=bidirectional, window_size=window_size,how=how).to(device)
elif dataset == 'HAR':
    label_size = 8
    ...
    if structure == 'lstm':
        ...
        mdl = model_transformer_HAR(batchsize=batch_size,num_classes=8,seq_len=window_size).to(device)
    elif structure == 'Conv_lstm':
        mdl = Conv_lstm_realworld(device=device,num_lstm_layers=num_layers,hidden_size=96,batch_size=batch_size).to(device)
